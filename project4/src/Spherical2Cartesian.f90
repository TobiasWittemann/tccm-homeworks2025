...
! Define angular momentum and allocate index list for cartesian exponents and transformation matrix
l = 2
allocate(index_list((l+1)*(l+2)/2,3))
allocate(M_trans((l+1)*(l+2)/2,-l:+l))


! Start calculation of transformation matrix
  ...
  do t = 0, floor(real((l-abs(m)),kind=8)/2)
    do u = 0, t
      do v_count = 0, floor(real(abs(m),kind=8)/2-vm)
        ...
        write(*,*) "exponents", exponents, "Nlm", Nlm, "C", C
